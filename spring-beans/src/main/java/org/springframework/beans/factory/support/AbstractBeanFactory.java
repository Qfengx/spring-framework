...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		 ...
		 * 	也就是将ObjectFactory加入到缓存中  一旦下个bean创建的时候需要依赖上个bean就会直接使用ObjectFactory
		 */
		// 直接尝试从缓存中提取 或者singletonFactory中的ObjectFactory中获取
		...
		if (sharedInstance != null && args == null) {
			...
			/**
			 * 只有单例的情况下才会尝试解决循环依赖 非单例 直接抛出异常
			 * 如果A中有B B中有A 那么依赖注入的时候会产生A还没创建完就要先创建B
			 */
			...
			try {
				/**
				 * 将存储XML配置文件中的FernericBeanDefiniton 转换为RootBeanDefiniton
	...
	protected Object getObjectForBeanInstance(
		...
		// 如果指定的name是工厂 相关(以&为前缀) 且beanInstance又不是FactoryBean类型则验证不通过
		if (BeanFactoryUtils.isFactoryDereference(name)) {
		 ...
		 * 如果是FactoruBean 我们使用它创建实例 但如果哟关于向直接获取工程实例而不是工厂的getObject对应实例
		 * 则 需要在beanName前加& 进行标识来获取
		 */
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
		// 这里说明是工厂  加载FactoryBean
		Object object = null;
		if (mbd == null) {
			// 尝试从缓存中加载bean
